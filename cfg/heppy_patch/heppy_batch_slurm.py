...
def batch_script_PSI_SLURM( index, jobDir, remoteDir=''):
    '''prepare a SLURM version of the batch script, to run on the PSI tier3 batch system'''
...
rm Loop/cmsswPreProcessing.root
cp -r Loop/* $SUBMISSIONDIR""".format(idx=index, srm='srm://t3se01.psi.ch'+remoteDir+jobDir[jobDir.rfind("/"):jobDir.find("_Chunk")])
    else:
...
export SCRAM_ARCH=slc7_amd64_gcc700 # NOT SURE IF THIS IS THE CORRECT ARCH (VS 10/14/19)
#cd $CMSSW_BASE/src
cd {cmssw}/src
shopt -s expand_aliases
cmsenv
...
class MyBatchManager( BatchManager ):
    ...
    def PrepareJobUser(self, jobDir, value ):
       ...
       if mode in ('LXPLUS-LSF', 'LXPLUS-CONDOR-SIMPLE', 'LXPLUS-CONDOR-TRANSFER'):
           ...
           scriptFile.write( batch_script_PSI_SLURM ( value, jobDir, storeDir ) ) # storeDir not implemented at the moment
       elif mode == 'LOCAL':
