...
# if options.runFullCfg:
if runFullCfg:
    lowPtEleCut  = 'pt>5 & abs(eta)<2.5 & ((%s) | electronID("mvaEleID_Fall17_noIso_V2_wp90") | electronID("mvaEleID_Fall17_iso_V2_wp90"))' %modified_loose_ele_id                                
    highPtEleCut = 'pt>25 & abs(eta)<2.5 & (electronID("mvaEleID_Fall17_noIso_V2_wp90") | electronID("mvaEleID_Fall17_iso_V2_wp90"))'
else:
    lowPtEleCut  = 'pt>5 & abs(eta)<2.5 & ((%s) | electronID("mvaEleID-Fall17-noIso-V2-wp90") | electronID("mvaEleID-Fall17-iso-V2-wp90"))' %modified_loose_ele_id                                
    highPtEleCut = 'pt>25 & abs(eta)<2.5 & (electronID("mvaEleID-Fall17-noIso-V2-wp90") | electronID("mvaEleID-Fall17-iso-V2-wp90"))'
...
process.goodHighPtEles = cms.EDFilter('PATElectronSelector',
    src = cms.InputTag('slimmedElectrons', '', process.name_()),
    cut = cms.string(highPtEleCut),
    filter = cms.bool(False)
)
             
process.goodLowPtEles = cms.EDFilter('PATElectronSelector',
    src = cms.InputTag('slimmedElectrons', '', process.name_()),
    cut = cms.string(lowPtEleCut),
    filter = cms.bool(False)
)
...
process.output = cms.OutputModule('PoolOutputModule',
    ...
    outputCommands = cms.untracked.vstring(
        ...
        'drop patPackedCandidates_packedPFCandidates__PAT'       ,
        'drop patMuons_slimmedMuons__PAT'                        ,
        'drop patElectrons_slimmedElectrons__PAT'                ,
        'drop patElectrons_slimmedElectrons__%s' %process.name_(),
        'drop patJets_slimmedJets__PAT'                          ,
        'drop patTaus_slimmedTaus__PAT'                          ,
        'drop patPhotons_slimmedPhotons__PAT'                    ,
        'drop patPhotons_slimmedOOTPhotons__PAT'                 ,
        'drop patJets_slimmedJetsPuppi__PAT'                     ,
        'drop patJets_slimmedJetsAK8__PAT'                       ,
        'drop patTaus_slimmedTausBoosted__PAT'                   ,
        'drop patIsolatedTracks_isolatedTracks__PAT'             ,
        'drop patPackedCandidates_lostTracks__PAT'               ,
        'drop recoGenJets_slimmedGenJetsAK8__PAT'                ,
        'drop l1tTauBXVector_caloStage2Digis_Tau_RECO'           ,
        'drop l1tJetBXVector_caloStage2Digis_Jet_RECO'           ,
        'drop l1tEtSumBXVector_caloStage2Digis_EtSum_RECO'       ,
        'drop l1tMuonBXVector_gmtStage2Digis_Muon_RECO'          ,
        'drop *_l1extraParticles_*_RECO'                         ,
        'keep patElectrons_goodLowPtEles__%s' %process.name_()   ,
        'keep patMuons_goodLowPtMuons__%s' %process.name_()      ,
    ),
