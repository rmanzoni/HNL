...
for ii, event in enumerate(events):
    ...
    event.getByLabel (label_mu, handle_mu)
    ...
    mus = handle_mu.product()
    nmus = len(mus)
    
    for e in eles: leps.append(e)
    ...
    print '%d leptons: %d electrons and %d muons'%(len(leps),neles,nmus)
    

    if nleps >1:
        for i in xrange(nleps):
            print '%d. pdgId = %d;\tpt = %.2f;'%(i,leps[i].pdgId(),leps[i].pt())    
            ...
            if pair[0] != pair [1]:
                for il in pair:
                    if abs(il.pdgId())==13:
                        ...
                        else: print 'vtx %d: could not load muonBestTrack() from the muon'%(index)
                    if abs(il.pdgId())==11:
                        if il.gsfTrack().get():
                            tofit.push_back(ROOT.reco.Track(il.gsfTrack().get()))
                            if not ROOT.reco.Track(il.gsfTrack().get()).numberOfValidHits()>0:
                                print 'vtx %d: there are no valid tracker hits in this electron track'%(index)
                        else: print 'vtx %d: could not load gsfTrack() from the electron'%(index)
                ...
                if tofit.size() == 2 and tofit[0]!=tofit[1]:
                    ...
                    if not sv.get().isEmpty() and sv.get().isValid():
                        ...
                        print 'vtx %d:\tpdgID(l1/l2) = %d/%d;\t\tpt(l1/l2) = %.3f/%.3f\t\tx= %.6f\txErr = %.6f\t Chi2 = %.6f'%(index,pair[0].pdgId(),pair[1].pdgId(),pair[0].pt(),pair[1].pt(),vtx.x(),vtx.xError(),vtx.chi2())
                        continue
                    else: 
